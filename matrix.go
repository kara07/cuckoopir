...
func (m Matrix) TransposeCopy() *Matrix {
	if m.Cols == 1 {
		m.Cols = m.Rows
		m.Rows = 1
		return &m
...
func (m *Matrix) SelectRow(i uint64) *Matrix {
	if m.Rows == 1 {
		return m
	}

	row := MatrixNew(1, m.Cols)
	for j := uint64(0); j < m.Cols; j++ {
		row.Data[j] = m.Data[(i-1) * m.Cols + j]
	}
	return row
}
...
func (m *Matrix) SelectSparseRows(rows []uint64) *Matrix {
	m2 := MatrixNewNoAlloc(0, m.Cols)
	for _, row_num := range rows{
		row := m.SelectRow(row_num)
		m2.Concat(row)
	}
