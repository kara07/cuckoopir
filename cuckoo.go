...
const (
	blen      = 1 << bshift             // number of key/value cells per bucket.
	bmask     = blen - 1				// mask for bucket index.
	nhash     = 1 << nhashshift			// number of hash functions.
	nhashmask = nhash - 1				// mask for hash function index.
)
...
func alloc(n int) []bucket {				// allocates a slice of buckets.
...
//merge values
func MergeValues(c *Cuckoo){
	var ValueMatrix []Value
